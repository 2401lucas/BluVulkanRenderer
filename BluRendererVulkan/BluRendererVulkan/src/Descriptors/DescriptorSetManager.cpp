...
DescriptorSetManager::DescriptorSetManager(Device* deviceInfo, Descriptor* descriptorSetLayout, ModelBufferManager* modelManager)
{
...
void DescriptorSetManager::createDescriptorSets(Device* deviceInfo, Descriptor* descriptorSetLayout, ModelBufferManager* modelManager)
{
    ...
    for (uint32_t i = 0; i < RenderConst::MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        imageInfo.imageView = modelManager->getModels(0)->getTexture()->getImageView();
        imageInfo.sampler = modelManager->getModels(0)->getTexture()->getImageSampler();
