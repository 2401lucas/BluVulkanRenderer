...
int BluRendererVulkan::run(int argc, char** argv) {
  ...
  deviceSettings.enabledDeviceFeatures.multiDrawIndirect = VK_TRUE;
  deviceSettings.enabledDeviceFeatures12.sType =
  ...
  deviceSettings.enabledDeviceFeatures12.pNext =
      &deviceSettings.enabledFragShaderBarycentricFeatures;
  deviceSettings.enabledFragShaderBarycentricFeatures.sType =
  ...
  engineCore = std::make_unique<EngineCore>(
      windowManager->getWindow(), appInfo,
      deviceSettings /*TODO: buildInfo(Scenes to load... ect)*/);
  ...
  while (isRunning) {
    ...
    engineCore->update(frameTime, windowManager->getInput(),
                       windowManager->isFramebufferResized());
