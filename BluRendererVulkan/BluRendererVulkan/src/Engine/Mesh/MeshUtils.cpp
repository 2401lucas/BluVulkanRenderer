...
void MeshUtils::getMeshDataFromPath(MeshData* meshData) {
  ...
  auto xExtremes =
      std::minmax_element(meshData->vertices.begin(), meshData->vertices.end(),
  ...
  glm::vec3 upperLeft = glm::vec3(
      (xExtremes.first->pos, yExtremes.first->pos, zExtremes.first->pos));
  glm::vec3 lowerRight = glm::vec3(
      (xExtremes.second->pos, yExtremes.second->pos, zExtremes.second->pos));
