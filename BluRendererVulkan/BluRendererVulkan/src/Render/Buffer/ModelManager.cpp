...
void ModelManager::loadModels(Device* deviceInfo, CommandPool* commandPool, const std::vector<SceneModel> modelCreateInfos) {
    for (uint32_t i = 0; i < modelCreateInfos.size(); i++) {
        models.push_back(new Model(modelCreateInfos[i], getTextureIndex(modelCreateInfos[i].texturePath), getTextureType(modelCreateInfos[i].texturePath)));
        modelData.push_back(PushConstantData(glm::vec4(models[i]->getTextureIndex(), i, models[i]->getMaterialIndex(), models[i]->getTextureType())));
    }
