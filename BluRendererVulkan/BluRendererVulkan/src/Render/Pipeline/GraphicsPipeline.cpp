...
GraphicsPipeline::GraphicsPipeline(Device* deviceInfo, const std::vector<ShaderInfo> shaders, const std::vector<VkDescriptorSetLayout>& descriptorLayouts, RenderPass* renderPass)
{
    ...
    multisampling.rasterizationSamples = deviceInfo->getMipSampleCount();
    ...
    if (vkCreateGraphicsPipelines(deviceInfo->getLogicalDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
