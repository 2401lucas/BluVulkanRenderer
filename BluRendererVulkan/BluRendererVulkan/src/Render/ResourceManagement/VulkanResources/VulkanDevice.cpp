 ...
 *
 * @param typeBits Bit mask with bits set for each memory type supported by the
 ...
 * SRS - support VkQueueFlags parameter for requesting multiple flags vs.
 * VkQueueFlagBits for a single flag only
 *
 ...
 * @param useSwapChain Set to false for headless rendering to omit the swapchain
 * device extensions
 ...
 * buffer is copied
 *
 * @note Source and destination pointers must have the appropriate transfer
 * usage flags set (TRANSFER_SRC / TRANSFER_DST)
 */
void VulkanDevice::copyBuffer(vks::Buffer *src, vks::Buffer *dst, VkQueue queue,
 ...
 *
 * @param queueFamilyIndex Family index of the queue to create the command pool
 * for
 ...
 * @param (Optional) begin If true, recording on the new command buffer will be
 * started (vkBeginCommandBuffer) (Defaults to false)
 *
...
VkCommandBuffer VulkanDevice::createCommandBuffer(VkCommandBufferLevel level,
  ...
  VkCommandBufferAllocateInfo cmdBufAllocateInfo =
      vks::initializers::commandBufferAllocateInfo(pool, level, 1);
 ...
 * (Defaults to true)
 *
 * @note The queue that the command buffer is submitted to must be from the same
 * family index as the pool it was allocated from
 ...
 *
 * @param checkSamplingSupport Check if the format can be sampled from (e.g. for
 * shader reads)
 *
