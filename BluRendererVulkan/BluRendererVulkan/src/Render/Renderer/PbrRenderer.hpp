...
#include "../ThreadPools/ThreadPool.hpp"
...
class ImGUI {
 ...
 public:
  ...
  void setupRenderPass(BaseRenderer* br) {
    std::array<VkAttachmentDescription, 2> attachments = {};
    // Color attachment
    attachments[0].format = br->swapChain.colorFormat;
    ...
    // Depth attachment
    attachments[1].format = br->depthFormat;
  ...
  void initResources(BaseRenderer* br, VkQueue copyQueue,
  ...
  void newFrame(BaseRenderer* renderer, bool updateFrameGraph) {
    ...
    if (ImGui::CollapsingHeader("Rendering Settings")) {
      ...
      if (ImGui::Combo("MSAA Samples", &uiSettings.msaaSamples,
                       "NO MSAA\0 2X MSAA\0 4X MSAA")) {
        switch (uiSettings.msaaSamples) {
          case 0:
            renderer->setMsaaSampleCount(VK_SAMPLE_COUNT_1_BIT);
            break;
          case 1:
            renderer->setMsaaSampleCount(VK_SAMPLE_COUNT_2_BIT);
            break;
          case 2:
            renderer->setMsaaSampleCount(VK_SAMPLE_COUNT_4_BIT);
            break;
        }
      }
    }
...
class PbrRenderer : public BaseRenderer {
 public:
  ...
  struct SecondaryCommandBuffers {
    std::vector<VkCommandBuffer> background{VK_NULL_HANDLE};
    std::vector<VkCommandBuffer> ui{VK_NULL_HANDLE};
  ...
  PbrRenderer(std::vector<const char*> args) : BaseRenderer(args) {
    ...
    // Instead of setting numObjectsPerThread, it should be
    // maxNumObjectsPerThread
    numObjectsPerThread = 512 / numThreads;  // TODO
  ...
  void createCommandBuffers() {
    ...
    secondaryCommandBuffers.background.resize(swapChain.imageCount);
    secondaryCommandBuffers.ui.resize(swapChain.imageCount);
    for (uint32_t i = 0; i < drawCmdBuffers.size(); i++) {
      VK_CHECK_RESULT(vkAllocateCommandBuffers(
          device, &cmdBufAllocateInfo, &secondaryCommandBuffers.background[i]));
      VK_CHECK_RESULT(vkAllocateCommandBuffers(device, &cmdBufAllocateInfo,
                                               &secondaryCommandBuffers.ui[i]));
    }
  }
    ...
    for (uint32_t i = 0; i < numThreads; i++) {
      ...
      VkCommandBufferAllocateInfo secondaryCmdBufAllocateInfo =
          vks::initializers::commandBufferAllocateInfo(
              ...
              static_cast<uint32_t>(thread->commandBuffer.size()));
      VK_CHECK_RESULT(vkAllocateCommandBuffers(
          device, &secondaryCmdBufAllocateInfo, thread->commandBuffer.data()));
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      if (uiSettings.cerberus) {
        ...
        vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                          pipelines.pbr);
        models.cerberus.draw(drawCmdBuffers[i]);
      }
  ...
  void buildSecondaryCommandBuffers(
    ...
    // Secondary Command Buffer for Skybox
    if (uiSettings.displaySkybox) {
      VK_CHECK_RESULT(vkBeginCommandBuffer(
          secondaryCommandBuffers.background[currentBuffer],
          &commandBufferBeginInfo));
      // Skybox
      vkCmdSetViewport(secondaryCommandBuffers.background[currentBuffer], 0, 1,
                       &viewport);
      vkCmdSetScissor(secondaryCommandBuffers.background[currentBuffer], 0, 1,
                      &scissor);

      vkCmdBindDescriptorSets(secondaryCommandBuffers.background[currentBuffer],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.skybox, 0, NULL);
      vkCmdBindPipeline(secondaryCommandBuffers.background[currentBuffer],
                        VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.skybox);
      models.skybox.draw(secondaryCommandBuffers.background[currentBuffer]);
      VK_CHECK_RESULT(vkEndCommandBuffer(
          secondaryCommandBuffers.background[currentBuffer]));
    }

    // ImGUI RenderPass
    // TODO: Bind RenderPass
    VK_CHECK_RESULT(vkBeginCommandBuffer(
        secondaryCommandBuffers.ui[currentBuffer], &commandBufferBeginInfo));
    vkCmdSetViewport(secondaryCommandBuffers.ui[currentBuffer], 0, 1,
                     &viewport);
    vkCmdSetScissor(secondaryCommandBuffers.ui[currentBuffer], 0, 1, &scissor);
    imGui->drawFrame(secondaryCommandBuffers.ui[currentBuffer]);
    VK_CHECK_RESULT(
        vkEndCommandBuffer(secondaryCommandBuffers.ui[currentBuffer]));
  }
  ...
  void prepareImGui() {
    ...
    imGui->initResources(this, queue, "Shaders/");  // TODO
  }
