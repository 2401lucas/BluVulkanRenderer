...
class ImGUI {
 ...
 public:
  ...
  void newFrame(BaseRenderer* example, bool updateFrameGraph) {
    ImGui::NewFrame();
    ...
    // Debug window
    ImGui::SetWindowPos(
        ImVec2(20 * example->UIOverlay.scale, 20 * example->UIOverlay.scale),
        ImGuiCond_FirstUseEver);
    ImGui::SetWindowSize(
        ImVec2(300 * example->UIOverlay.scale, 300 * example->UIOverlay.scale),
        ImGuiCond_Always);
    ImGui::TextUnformatted(example->title.c_str());
    ImGui::TextUnformatted(device->properties.deviceName);

    // SRS - Display Vulkan API version and device driver information if
    ...
    ImGui::SetNextWindowPos(
        ImVec2(20 * example->UIOverlay.scale, 360 * example->UIOverlay.scale),
        ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(
        ImVec2(300 * example->UIOverlay.scale, 200 * example->UIOverlay.scale),
        ImGuiCond_FirstUseEver);
...
class VulkanExample : public BaseRenderer {
 public:
  ImGUI* imGui = nullptr;
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader("shaders/scene.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader("shaders/scene.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
  ...
  void prepareImGui() {
    ...
    imGui->initResources(renderPass, queue, "Shaders/");  // TODO
  }
