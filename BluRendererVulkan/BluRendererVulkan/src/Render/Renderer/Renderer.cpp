...
Renderer::Renderer(VulkanInstance* vkInstance, Device* device, const SceneDependancies& sceneDependancies)
{
...
void Renderer::createSyncObjects(Device* device) {
...
void Renderer::drawFrame(Device* device, const bool& framebufferResized, const SceneInfo* sceneInfo)
{
    ...
    graphicsPipelines[0]->bindPipeline(currentCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS);
    graphicsPipelines[0]->bindDescriptorSets(currentCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, 1, descriptorManager->getGlobalDescriptorSet(frameIndex), 0, nullptr);
    graphicsPipelines[0]->bindDescriptorSets(currentCommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, 1, 1, descriptorManager->getMaterialDescriptorSet(frameIndex), 0, nullptr);

    auto modelCount = modelManager->getModelCount();
    for (int i = 0; i < modelCount; i++)
    {
        modelManager->bindBuffers(currentCommandBuffer, i);
        modelManager->updatePushConstants(currentCommandBuffer, graphicsPipelines[0]->getPipelineLayout(), i);
